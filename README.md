...
| File | Pass | Purpose |
|---|---|---|
| `00_StockNames.cfg` | `:FINAL` | Renames the 31 bodies and every JNSQ-Reborn config that refers to them: Kopernicus bodies and orbits, asteroid spawn locations, home world, EVE clouds/shadows/PQS/wet surfaces/city lights, Scatterer atmospheres, oceans and planet list (eclipse casters, secondary suns, light sources), Parallax terrain and scatters, Kerbal Konstructs instances, group centers and map decals, NavInstruments runways, MechJeb sites, Distant Object, PlanetShine, Firefly, Kerbalism, Rational Resources / CRP, Deployed Science, CommNet Constellation, ROC definitions, contract settings, ResearchBodies. |
| `01_DedupeLeftovers.cfg` | `:LAST[JNSQ-Reborn]` | JNSQ-Reborn requires classic JNSQ to stay installed and relies on its stock-named configs (EVE eclipses, Scatterer atmospheres, Kerbal Konstructs sites, MechJeb sites, ...) pointing at bodies that no longer exist. With the stock names back they would load twice. This file removes them while the Reborn copies still carry their `JNSQ*` names, so only the leftovers match. Kerbal Konstructs statics are removed only for the 27 site groups shipped by classic JNSQ, so anything you placed yourself is left alone. |
...
- JNSQ-Reborn's Kerbal Konstructs sites use the models of Omega's Stockalike Structures, Kerbin Side Remastered, Tundra's Space Center and Ordinary Konstruction Co. Without those packs Kerbal Konstructs logs one "No Model named ..." line per missing static, with or without this patch.
...
```bash
python tools/generate.py --reborn <KSP>/GameData/JNSQ-Reborn --jnsq <KSP>/GameData/JNSQ
